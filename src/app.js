...
import { INTEREST_RATES } from './data/interestRates.js';
...
const parseCompetencia = (value) => {
  ...
  if (!mm || !yyyy || yyyy.length !== 4) return null;
...
const getInterestRate = (year) => {
  const years = Object.keys(INTEREST_RATES)
    .map(Number)
    .sort((a, b) => a - b);
  if (INTEREST_RATES[year]) {
    return { rate: INTEREST_RATES[year], rule: `Taxa do exercício ${year}` };
  }
  const previous = years.filter((y) => y < year).pop();
  if (previous) {
    return { rate: INTEREST_RATES[previous], rule: `Taxa do último exercício disponível (${previous})` };
  }
  return { rate: years.length ? INTEREST_RATES[years[0]] : 0, rule: 'Taxa indisponível na base' };
};

const getInpcIndex = (key, warnings) => {
  if (INPC_INDEX[key]) return INPC_INDEX[key];
  const latestKey = INPC_RANGE.end;
  if (key > latestKey) {
    warnings.push(`Competência final ${key} fora da série. Usado INPC mais recente (${latestKey}).`);
    return INPC_INDEX[latestKey];
  }
  warnings.push(`Competência ${key} não encontrada na série INPC.`);
  return null;
};
...
const runTests = () => {
  ...
  const ax = calculateAx12(58, 0.0437, 'F');
  testItems.push({
...
äx(12): ${formatNumber(data.ax12, 5)}
...
Aplicou-se VAEBA = NSUA × SUP × äx(12) × FATCOR, com NSUA=13, FCB=0,9818, crescimento real 0% a.a., FATCOR via INPC e tábua AT-2000 Suavizada (10%).
...
const calculate = () => {
  ...
  if (!competenciaBase) {
    warnings.push('Competência base INPC inválida ou não informada.');
  }
  ...
  if (!competenciaFinal) {
    warnings.push('Competência final INPC inválida.');
  }

  const inpcBase = competenciaBase ? getInpcIndex(competenciaBase.key, warnings) : null;
  const inpcFinal = competenciaFinal ? getInpcIndex(competenciaFinal.key, warnings) : null;
  ...
  const { rate, rule } = getInterestRate(dataCalculo ? new Date(dataCalculo).getFullYear() : new Date().getFullYear());
  if (!INTEREST_RATES[new Date(dataCalculo || new Date()).getFullYear()]) {
    warnings.push(`Taxa de juros não encontrada para ${new Date(dataCalculo || new Date()).getFullYear()}. Usada taxa anterior.`);
  }

  const ax12 = idade !== null && sexo ? calculateAx12(idade, rate, sexo) : 0;

  const vaebaBruta = NSUA * beneficioBruto * ax12 * fatcor;
  const vaebaAjustada = NSUA * supAjustado * ax12 * fatcor;

  outputs.fatcor.textContent = formatNumber(fatcor, 5);
  outputs.ax12.textContent = formatNumber(ax12, 5);
